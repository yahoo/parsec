...
public class ParsecExceptionMapper implements ExceptionMapper<RuntimeException> {
    ...
    private void logRuntimeException(RuntimeException e, int httpStatus) {
        int internalServerErrorCode = Response.Status.INTERNAL_SERVER_ERROR.getStatusCode();
        if ((httpStatus == internalServerErrorCode && LOG.isErrorEnabled()) || LOG.isDebugEnabled()) {
            Throwable rootCause = Throwables.getRootCause(e);
            ...
            meta.put("uri", request == null ? "" : request.getRequestURL().toString());
            meta.put("trace_string", errStack);
            String logInfo = LogUtil.generateLog(className, msg, meta);
            if (httpStatus == internalServerErrorCode) {
                LOG.error(logInfo);
            } else {
                LOG.debug(logInfo);
            }
